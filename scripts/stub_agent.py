...
STUB_PATH = "stubs/"
SYMBOL_FILE = "nautilus_trader/nautilus_trader/core/nautilus_pyo3.pyi"

VALIDATION_SUCCESS = "All validations passed!"
DEFAULT_MODEL = "openrouter/qwen/qwen3.5-plus-02-15"  #"openrouter/openai/gpt-4o"
DEFAULT_MAX_RETRIES = 3
CONTEXT_LINES = 25          # lines of context around each error location
MAX_TOOL_CALLS = 50         # guard against runaway tool-call loops
...
def tool_insert_edit_into_file(
    file_path: str,
    start_line: int,
    end_line: int,
    new_content: str,
...
def tool_diff(original: str, modified: str, filename: str = "file") -> dict:
    import difflib
...
def execute_tool_call(name: str, args: dict) -> dict:
    dispatch = {
        ...
        "insert_edit_into_file": tool_insert_edit_into_file,
        "diff": tool_diff,
    }
...
def run_validation(pyx_file: Path, pyi_file: Path) -> tuple[bool, str]:
    ...
    return VALIDATION_SUCCESS in output, output


def _extract_line_numbers(validation_output: str, filename: str) -> list[int]:
    pattern = rf'\({re.escape(filename)}:(\d+)\)'
    return sorted({int(m) for m in re.findall(pattern, validation_output)})


def _format_section(file_path: Path, line_numbers: list[int], context: int = CONTEXT_LINES) -> str:
...
def _build_generation_prompt(symbol_code: str, cython_code: str) -> str:
...
def generate_stub(pyx_file: Path, symbol_code: str, model: str) -> str:
    """Call the LLM to generate a .pyi stub from a .pyx file."""
    prompt = _build_generation_prompt(symbol_code, pyx_file.read_text(encoding="utf-8"))
...
def fix_stub(
    ...
    """
    pyi_lines = _extract_line_numbers(validation_output, pyi_file.name)
    pyx_lines = _extract_line_numbers(validation_output, pyx_file.name)

    pyi_section = _format_section(pyi_file, pyi_lines)
    pyx_section = _format_section(pyx_file, pyx_lines) if pyx_lines else None
    ...
    while calls_made < MAX_TOOL_CALLS:
        ...
        messages.append(_serialize_message(resp_msg))
    ...
    if calls_made >= MAX_TOOL_CALLS:
        print(f"    ⚠ Reached MAX_TOOL_CALLS ({MAX_TOOL_CALLS}) during fix.")


# ──────────────────────────────────────────────────────────────
# Orphaned stub cleanup
# ──────────────────────────────────────────────────────────────

def cleanup_orphaned_stubs(module_root: Path, stub_root: Path) -> list[Path]:
    """
    ...
    for pyi_file in sorted(stub_root.rglob("*.pyi")):
        relative = pyi_file.relative_to(stub_root)
        pyx_file = module_root / relative.with_suffix(".pyx")
        if not pyx_file.exists():
            print(f"  Deleting orphaned stub (no .pyx): {pyi_file.relative_to(stub_root.parent)}")
            pyi_file.unlink()
            deleted.append(pyi_file)
            # Prune empty ancestor directories up to stub_root
            parent = pyi_file.parent
            while parent != stub_root:
                try:
...
def process_file(
    ...
    if is_new:
        ...
        content = generate_stub(pyx_file, symbol_code, model)
    ...
    passed, output = run_validation(pyx_file, pyi_file)
    if verbose:
        print(f"  Validation:\n{output}\n")
    if passed:
    ...
    for attempt in range(1, max_retries + 1):
        ...
        fix_stub(pyx_file, pyi_file, output, model, verbose=verbose)
...
def main() -> None:
    ...
    parser.add_argument(
        "--model",
        default=os.environ.get("STUB_AGENT_MODEL", DEFAULT_MODEL),
    ...
    symbol_code = symbol_file.read_text(encoding="utf-8") if symbol_file.exists() else ""
    if not symbol_code:
        print(f"Warning: symbol file not found at {symbol_file}", file=sys.stderr)

    # Collect .pyx files to process
    if args.pyx_file:
        pyx_files = [args.pyx_file.resolve()]
    else:
        pyx_files = [
            Path(p).resolve()
            for p in glob(str(PROJECT_ROOT / args.module_path) + "/**/*.pyx", recursive=True)
        ]
    ...
    module_root = (PROJECT_ROOT / args.module_path).resolve()
    stub_root = (PROJECT_ROOT / args.stub_path).resolve()

    # ── Clean up stubs that have no corresponding .pyx ────────
    print("Scanning for orphaned stubs...")
    deleted = cleanup_orphaned_stubs(module_root, stub_root)
    if deleted:
    ...
    for idx, pyx_file in enumerate(pyx_files, 1):
        ...
        print(f"[{idx}/{len(pyx_files)}] {pyx_file.relative_to(PROJECT_ROOT)}")

        try:
            success, status = process_file(
                pyx_file=pyx_file,
                pyi_file=pyi_file,
